...
/* disposes of A */
{
    ...
    A1 = sf_sort(A,
                 (qsort_compare_func)descend); /* sort into descending order */
...
int d1_rm_equal(pset *A1, qsort_compare_func compare)
...
/* updated in place */
{
    ...
    for (pa = A1; (a = *pa++) != NULL;) {
        ...
        if ((int)SIZE(a) != last_size) last_size = SIZE(a), pcheck = pdest;
        for (pb = A1; pb != pcheck;) {
            b = *pb++;
...
int rm2_equal(pset *A1, pset *B1, pset *E1, qsort_compare_func compare)
/* updated in place */

{
    ...
    for (; *A1 != NULL && *B1 != NULL;) switch ((*compare)(A1, B1)) {
    ...
    while (*A1 != NULL) *pda++ = *A1++;
    while (*B1 != NULL) *pdb++ = *B1++;
    *pda = *pdb = *pde = NULL;
...
pset_family sf_ind_unlist(pset *A1, int totcnt, int size, int *row_indices,
    ...
    for (i = 0; i < totcnt; i++) row_indices[i] = new_row_indices[i];
