...
static bool simp_comp_special_cases(pset *T, pset_family *Tnew,
                                    pset_family *Tbar);
...
/* T will be disposed of */
{
    ...
    if (debug & COMPL) debug_print(T, "COMPLEMENT", compl_level++);
    ...
    if (debug & COMPL) debug1_print(Tbar, "exit COMPLEMENT", --compl_level);
    return Tbar;
}
...

{
    ...
    if (debug & COMPL) debug_print(T, "SIMPCOMP", simplify_level++);
