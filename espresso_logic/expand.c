...
int most_frequent(pset_family CC, pset FREESET) {
    ...
    for (i = 0; i < cube.size; i++) count[i] = 0;
    if (CC != (pcover)NULL)
...
void select_feasible(pset_family BB, pset_family CC, pset RAISE, pset FREESET,
    ...
    for (i = 0; i < numfeas; i++) feas_new_lower[i] = GETSET(new_lower, i);
...
#ifdef NEW
        for (j = 0; j < numfeas; j++)
            if (setp_disjoint(feas_new_lower[i], feas[j])) count++;
#else
        for (j = 0; j < numfeas; j++)
            if (setp_implies(feas[j], feas[i])) count++;
#endif
        if (count > bestcount) {
            bestcount = count;
            bestfeas = feas[i];
