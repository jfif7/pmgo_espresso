    ...
    { /* Check the multiple-valued variables */
        ...
        for (var = cube.num_binary_vars; var < cube.num_vars; var++) {
            ...
            for (w = cube.first_word[var]; w <= last; w++)
                if (a[w] & b[w] & mask[w]) goto nextvar;
    ...
    { /* Check binary variables */
        ...
        if ((last = cube.inword) != -1) {
            ...
            if ((x = ~(x | x >> 1) & cube.inmask))
                if ((dist = count_ones(x)) > 1) return 2;

            /* Check the full words of binary variables */
            for (w = 1; w < last; w++) {
                x = a[w] & b[w];
                if ((x = ~(x | x >> 1) & DISJOINT))
                    if (dist == 1 || (dist += count_ones(x)) > 1) return 2;
            }
        }
    }
...
int cdist(register pset a, register pset b) {
    ...
    { /* Check binary variables */
        ...
        if ((last = cube.inword) != -1) {
            /* Check the partial word of binary variables */
            x = a[last] & b[last];
            if ((x = ~(x | x >> 1) & cube.inmask)) dist = count_ones(x);

            /* Check the full words of binary variables */
            for (w = 1; w < last; w++) {
                x = a[w] & b[w];
                if ((x = ~(x | x >> 1) & DISJOINT)) dist += count_ones(x);
            }
        }
    }
...
void consensus(pset r, register pset a, register pset b) {
    ...
    { /* Check the multiple-valued variables */
        ...
        for (var = cube.num_binary_vars; var < cube.num_vars; var++) {
            ...
            empty = TRUE;
            for (w = cube.first_word[var]; w <= last; w++)
                if ((x = a[w] & b[w] & mask[w])) empty = FALSE, r[w] |= x;
            if (empty)
                for (w = cube.first_word[var]; w <= last; w++)
                    r[w] |= mask[w] & (a[w] | b[w]);
        }
...
bool ccommon(register pset a, register pset b, register pset cof) {
    { /* Check binary variables */
        ...
        if ((last = cube.inword) != -1) {
            ...
            x = a[last] | cof[last];
            y = b[last] | cof[last];
            if (~(x & x >> 1) & ~(y & y >> 1) & cube.inmask) return TRUE;

            /* Check the full words of binary variables */
            for (w = 1; w < last; w++) {
                x = a[w] | cof[w];
                y = b[w] | cof[w];
                if (~(x & x >> 1) & ~(y & y >> 1) & DISJOINT) return TRUE;
            }
        }
    }
