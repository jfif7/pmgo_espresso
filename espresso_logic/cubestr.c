...
void cube_setup(void) {
    ...
    for (var = 0; var < cube.num_vars; var++) {
        if (var < cube.num_binary_vars) cube.part_size[var] = 2;
    ...
    for (i = 0; i < CUBE_TEMP; i++) cube.temp[i] = new_cube();
...
void setdown_cube(void) {
    ...
    for (var = 0; var < cube.num_vars; var++) free_cube(cube.var_mask[var]);
    FREE(cube.var_mask);

    for (i = 0; i < CUBE_TEMP; i++) free_cube(cube.temp[i]);
    FREE(cube.temp);
