...
void set_pair1(pPLA PLA, int adjust_labels) {
    ...
    if (adjust_labels) makeup_labels(PLA);
  ...
  variables are left unpaired
    */
    ...
    for (var = 0; var < cube.num_binary_vars; var++) paired[var] = FALSE;
    ...
    if (adjust_labels) {
        ...
        for (i = 0; i < old_size; i++)
            if (oldlabel[i] != (char *)NULL) FREE(oldlabel[i]);
        FREE(oldlabel);
    }
    ...
    foreach_set(A, last, p) {
        for (pairnum = 0; pairnum < pair->cnt; pairnum++) {
            ...
            if (/* a0 */ is_in_set(p, p1)) {
                if (b0) set_insert(p, val + 3);
                if (b1) set_insert(p, val + 2);
            }
            if (/* a1 */ is_in_set(p, p1 + 1)) {
                if (b0) set_insert(p, val + 1);
                if (b1) set_insert(p, val);
            }
        }
    }
    return A;
...
pcover delvar(pset_family A, int *paired) {
    ...
    if (run) A = sf_delcol(A, first_run - offset, run_length);
    return A;
}
...
void find_optimal_pairing(pPLA PLA, int strategy) {
    ...
    if (summary) {
        printf("    ");
        for (i = 0; i < cube.num_binary_vars; i++) printf("%3d ", i + 1);
        printf("\n");
    ...
    for (i = 0; i < cube.num_binary_vars; i++) FREE(cost_array[i]);
    FREE(cost_array);
...
int **find_pairing_cost(pPLA PLA, int strategy) {
    ...
    for (i = 0; i < cube.num_binary_vars; i++)
        for (j = 0; j < cube.num_binary_vars; j++) cost_array[i][j] = 0;
...
void minimize_pair(ppair pair) {
    ...
    xnum_vars = cube.num_vars;
    xpart_size = ALLOC(int, cube.num_vars);
    for (i = 0; i < cube.num_vars; i++) xpart_size[i] = cube.part_size[i];
...
void generate_all_pairs(ppair pair, int n, pset candidate,
    ...
    for (i = 0; i < n; i++)
        if (is_in_set(candidate, i)) break;
