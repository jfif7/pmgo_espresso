...
void massive_count(pset *T) {
    ...
    {
        register int i;
        for (i = cube.size - 1; i >= 0; i--) count[i] = 0;
    }
        ...
        for (var = 0; var < cube.num_vars; var++) {
            ...
            if (active > mostactive)
                ...
                if (active == mostactive) {
                    ...
                    /* for binary variables, this is the same as minimum # of
                     * 2's */
                    if (cdata.var_zeros[var] > mostzero)
                        ...
                        if (cdata.var_zeros[var] == mostzero) {
                            ...
                            /* for binary vars, this means roughly equal # 0's
                             * and 1's */
...
int binate_split_select(pset *T, register pset cleft, register pset cright,
                        int debug_flag) {
    int best = cdata.best;
    ...
    for (i = cube.first_part[best]; i <= lastbit; i++)
        if (!is_in_set(cof, i)) halfbit++;
    for (i = cube.first_part[best], halfbit = halfbit / 2; halfbit > 0; i++)
        if (!is_in_set(cof, i)) halfbit--, set_insert(cleft, i);
    for (; i <= lastbit; i++)
        if (!is_in_set(cof, i)) set_insert(cright, i);

    if (debug & debug_flag) {
        printf("BINATE_SPLIT_SELECT: split against %d\n", best);
        if (verbose_debug) printf("cl=%s\ncr=%s\n", pc1(cleft), pc2(cright));
    }
    return best;
}
