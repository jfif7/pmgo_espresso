...
int set_dist(pset a, pset b) {
    ...
    unsigned int val;
    for (i = LOOP(a); i > 0; i--)
        if ((val = a[i] & b[i]) != 0) sum += count_ones(val);
    return sum;
}
...
pset_family sf_read(FILE *fp) {
    ...
    foreach_set(A, last, p) {
        ...
        for (j = 1; j <= (int)LOOP(p); j++) (void)fscanf(fp, "%x", p + j);
    }
...
char *pbv1(pset s, int n) {
    int i;
    for (i = 0; i < n; i++) s1[i] = is_in_set(s, i) ? '1' : '0';
    s1[n] = '\0';
    return s1;
}
...
pset_family sf_delcol(pset_family A, int firstcol, int n) {
    ...
    foreach_set(A, last, p) {
        ...
        for (i = 0; i < firstcol; i++)
            if (is_in_set(p, i)) set_insert(pdest, i);
        for (i = n > 0 ? firstcol + n : firstcol; i < A->sf_size; i++)
            if (is_in_set(p, i)) set_insert(pdest, i - n);
    }
    sf_free(A);
    return B;
}
...
pset_family sf_copy_col(pset_family dst, int dstcol, pset_family src,
    ...
    foreach_set(src, last, p) {
        if ((p[word_test] & bit_test) != 0) pdest[word_set] |= bit_set;
        /*
...
pset_family sf_permute(pset_family A, int *permute, int npermute) {
    ...
    foreach_set(A, last, p) {
        for (j = 0; j < npermute; j++)
            if (is_in_set(p, permute[j])) set_insert(pdest, j);
        pdest += B->wsize;
    }
    sf_free(A);
    return B;
