        ...
        switch (getc(fp)) {
            ...
            case '\n':
                if (!line_length_error)
                    fprintf(stderr,
                            "product term(s) span more than one line (warning "
                            "only)\n");
            ...
            for (i = cube.first_part[var]; i <= cube.last_part[var]; i++) {
                switch (getc(fp)) {
                    ...
                    case '\n':
                        if (!line_length_error)
                            fprintf(stderr,
                                    "product term(s) span more than one line "
                                    "(warning only)\n");
        ...
        case '.':
            ...
            if (str_equal(get_word(fp, word), "i")) {
                ...
                if (cube.fullset != NULL) {
                    ...
                    if (cube.part_size != NULL) fatal("cannot mix .i and .mv");
            ...
            /* .type specifies a logical type for the PLA */
                ...
                if (echo_unknown_commands) printf("%c%s ", ch, word);
...
void PLA_summary(pPLA PLA) {
    ...
    if (PLA->phase != NULL) printf("# phase is %s\n", pc1(PLA->phase));
    if (PLA->pair != NULL) {
...
void PLA_labels(pPLA PLA) {
    ...
    for (i = 0; i < cube.size; i++) PLA->label[i] = (char *)NULL;
}
...
void free_PLA(pPLA PLA) {
    ...
    if (PLA->F != (pcover)NULL) free_cover(PLA->F);

    if (PLA->R != (pcover)NULL) free_cover(PLA->R);

    if (PLA->D != (pcover)NULL) free_cover(PLA->D);

    if (PLA->phase != (pcube)NULL) free_cube(PLA->phase);
    ...
    if (PLA->label != NULL) {
        for (i = 0; i < cube.size; i++)
            if (PLA->label[i] != NULL) FREE(PLA->label[i]);
